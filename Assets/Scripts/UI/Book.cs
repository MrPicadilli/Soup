...
public class Book : MonoBehaviour {
    ...
    public void DragRightPageToPoint(Vector3 point)
    {
        ...
        if (currentPage < bookPages.Length)
            Left.sprite = (recette[currentPage]) ? bookPages[currentPage] : whitePages[1];
        else
        ...
        if(currentPage < bookPages.Length - 1)
            Right.sprite = (recette[currentPage + 1]) ? bookPages[currentPage + 1] : whitePages[0];
        else
            Right.sprite = background;

        if(currentPage < bookPages.Length - 2)
            RightNext.sprite = (recette[currentPage + 2]) ? bookPages[currentPage + 2] : whitePages[1];
        else
    ...
    public void DragLeftPageToPoint(Vector3 point)
    {
        ...
        Left.transform.eulerAngles = new Vector3(0, 0, 0);
        if (currentPage >= 2)
            Left.sprite = (recette[currentPage - 2]) ? bookPages[currentPage - 2] : whitePages[1];
        else
            Left.sprite = background;

        if (currentPage >= 3)
            LeftNext.sprite = (recette[currentPage - 3]) ? bookPages[currentPage - 3] : whitePages[0];
        else
    ...
    void UpdateSprites()
    {
        if (currentPage > 0 && currentPage <= bookPages.Length)
            LeftNext.sprite=(recette[currentPage - 1]) ? bookPages[currentPage - 1] : whitePages[0];
        else
            LeftNext.sprite = background;

        if (currentPage >= 0 && currentPage < bookPages.Length)
            RightNext.sprite = (recette[currentPage]) ? bookPages[currentPage]: whitePages[1];
        else
            RightNext.sprite = background;
    }
